...
public class BlockAnimationHandler {
    ...
    public static void onClientTick(TickEvent.ClientTickEvent event) {
        ...
        if (player == null) { lastRegisteredPlayer = null; return; }

        if (player != lastRegisteredPlayer) {
            try {
                PlayerAnimationAccess.getPlayerAnimLayer(player)
                        .addAnimLayer(10, SaberBlockAnimation.INSTANCE);
                lastRegisteredPlayer = player;
            } catch (Exception e) {
                UHeroesMod.LOGGER.warn("[U-Heroes] Could not register block layer: {}", e.getMessage());
                return;
            }
        }
